...
InputParameters validParams<DeformationParakeetAction>()
{
  ...
  params.addParam<std::vector<SubdomainName> >("block", "The list of ids of the blocks (subdomain) that the stress divergence kernel will be applied to");
   // we comment this line out since we add other AuxVariables.
...
DeformationParakeetAction::DeformationParakeetAction(const InputParameters & params) :
    Action(params),
...
DeformationParakeetAction::act()
{
  ...
  for (unsigned int i = 0; i < _ndisp; ++i)
  {
    std::string kernel_name = "DeformationParakeet_" + Moose::stringify(i);
...
std::string
DeformationParakeetAction::getKernelType()
{
  std::string type;
...
InputParameters
DeformationParakeetAction::getParameters(std::string type)
{
